  ...
  return (
    <div className="flex items-end gap-[3px] h-8" aria-hidden="true">
      {data.map((v, i) => (
        <div
          key={i}
          className={`flex-1 rounded-sm ${i === data.length - 1 ? "bg-primary" : "bg-muted-foreground/20"}`}
...
export function EnhancedMetricCard({ metric, currency, index }: EnhancedMetricCardProps & { index?: number }) {
  ...
  const content = (
    <CardContent className="p-5 h-full flex flex-col">
      ...
      <p className="text-2xl font-semibold tracking-tight tabular-nums mt-1">{formattedValue}</p>

      <div className="flex items-center gap-1.5 mt-1 min-h-[20px]">
        {hasChange && !isNeutral && (
          <span className={`inline-flex items-center gap-0.5 text-xs font-medium tabular-nums ${isPositive ? "text-emerald-600 dark:text-emerald-400" : "text-destructive"}`}>
  ...
  return (
    <Card className="h-full" role="article" aria-label={ariaLabel}>{content}</Card>
  );
}
