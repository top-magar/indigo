                            ...
                            {formData.name && (
                                <Card className="bg-muted/30 border-dashed">
                                    ...
                                    <CardContent className="space-y-2">
                                        ...
                                        {(() => {
                                            const enabledWithPrice = formData.variants.filter(v => v.enabled && v.price);
                                            if (enabledWithPrice.length === 0) return (
                                                <span className="text-sm text-muted-foreground">Set price in step 3</span>
                                            );
                                            const prices = enabledWithPrice.map(v => parseFloat(v.price));
                                        ...
                                        {formData.hasVariants && (
                                            <p className="text-xs text-muted-foreground">
                                                {formData.variants.filter(v => v.enabled).length} variant{formData.variants.filter(v => v.enabled).length !== 1 ? "s" : ""}
                                            </p>
                                        )}
                                    </CardContent>
                                </Card>
                                ...
                                <CardContent className="pt-4">
                                    ...
                                    <div className="mt-2 space-y-1">
                                        {[
                                            ...
                                            { done: formData.variants.some(v => v.enabled && !!v.price), label: "Set price", step: 2 as WizardStep },
