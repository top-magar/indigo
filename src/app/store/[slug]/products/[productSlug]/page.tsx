      ...
      {isDraftMode && (
        <div className="fixed bottom-4 right-4 z-50 flex items-center gap-2 rounded-lg bg-amber-400 px-4 py-2 text-sm font-medium text-white shadow-lg">
          ...
          {product.status === "draft" && (
            <span className="rounded bg-amber-500 px-2 py-0.5 text-xs">Draft Product</span>
          )}
          <a
...
export async function generateMetadata({
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `${process.env.NEXT_PUBLIC_APP_URL}/store/${slug}/products/${productSlug}`,
      languages: { "en-NP": `${process.env.NEXT_PUBLIC_APP_URL}/store/${slug}/products/${productSlug}` },
    },
    openGraph: {
      title: product.name,
      description,
