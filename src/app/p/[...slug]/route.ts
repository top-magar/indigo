...
function buildNav(pages: { name: string; slug: string; isHomepage: boolean | null }[], projectSlug: string) {
  return `<nav style="display:flex;gap:24px;padding:12px 24px;background:#fff;border-bottom:1px solid #eee;font-family:Inter,system-ui,sans-serif;font-size:14px">
    ${pages.map(p => `<a href="/p/${projectSlug}${p.isHomepage ? '' : `/${p.slug}`}" style="color:#333;text-decoration:none;font-weight:${p.isHomepage ? '600' : '400'}">${p.name}</a>`).join('')}
  </nav>`;
}
...
export async function GET(_req: Request, { params }: { params: Promise<{ slug: string[] }> }) {
  ...
  const pageSlug = segments[1]; // undefined = homepage
  ...
  const pages = await db.select({
    id: editorPages.id, name: editorPages.name, slug: editorPages.slug,
  ...
  // Build full HTML with navigation
  const nav = pages.length > 1 ? buildNav(pages, projectSlug) : '';
  const html = page.publishedHtml.replace(/<body[^>]*>/, (match) => `${match}${nav}`);
