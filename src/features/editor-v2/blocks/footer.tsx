        ...
        {cols.map((col, i) => (
          <div key={i}>
            ...
            <ul className="space-y-2">
              {col.links.map((l, j) => <li key={j}><a href={l.url} className="text-sm text-gray-600 hover:text-black">{l.label}</a></li>)}
            </ul>
          </div>
        ))}
      </div>
      {copyright && <p className="mt-8 border-t border-gray-200 pt-4 text-center text-sm text-gray-500">{copyright}</p>}
    </footer>
  )
}
