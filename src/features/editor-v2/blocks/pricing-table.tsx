      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: `repeat(${items.length || 1}, minmax(0, 1fr))` }}>
        {items.map((t, i) => (
          <div key={i} className={`rounded-lg border-2 p-6 ${t.highlighted ? "border-black" : "border-gray-200"}`}>
            ...
            <p className="mt-2 text-3xl" style={{ fontWeight: "var(--store-heading-weight, 700)", color: "var(--store-color-text, #0f172a)" }}>{t.price}</p>
            <ul className="mt-4 space-y-2">
              {t.features.map((f, j) => <li key={j} className="text-sm" style={{ color: "var(--store-color-muted, #64748b)" }}>✓ {f}</li>)}
            </ul>
