...
export function Popup({ heading, content, buttonText, trigger, timerDelay, scrollPercent, __isEditor }: PopupProps) {
  ...
  if (__isEditor) {
    return (
      <div className="rounded-xl border-2 border-dashed border-gray-300 p-6">
        ...
        <div className="text-sm text-gray-600" dangerouslySetInnerHTML={{ __html: content }} />
  ...
  return (
    <>
      {trigger === "button" && <button onClick={() => ref.current?.showModal()} className="rounded bg-black px-4 py-2 text-sm text-white">{buttonText || "Open"}</button>}
      <dialog ref={ref} className="max-w-md rounded-xl p-6 backdrop:bg-black/50">
        <h3 className="mb-2 text-lg font-bold">{heading}</h3>
        <div className="text-sm text-gray-600" dangerouslySetInnerHTML={{ __html: content }} />
        <button onClick={() => ref.current?.close()} className="mt-4 rounded bg-black px-4 py-2 text-sm text-white">{buttonText || "Close"}</button>
      </dialog>
    </>
  )
}
