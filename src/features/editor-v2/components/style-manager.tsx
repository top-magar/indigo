...
function useStyleProp(sectionId: string, key: StyleKey, fallback: string | number) {
  const value = useEditorStore((s) => {
    ...
    // Check viewport override first, then base
    if (viewport !== "desktop") {
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <div className="flex gap-2">
        <input type="color" value={(value as string) || "#ffffff"} onChange={(e) => update(e.target.value)} className="h-7 w-7 rounded border cursor-pointer shrink-0" />
...
function SelectField({ sectionId, prop, label, options }: { sectionId: string; prop: StyleKey; label: string; options: { value: string; label: string }[] }) {
  ...
  return (
    <div className="flex flex-col gap-1">
      <Label className="text-xs">{label}</Label>
      <select value={value as string} onChange={(e) => update(e.target.value)} className="h-7 rounded border bg-background px-2 text-xs">
...
export function StyleManager({ sectionId }: { sectionId: string }) {
  ...
  return (
    <div className="flex flex-col gap-1">
      {viewport !== "desktop" && (
        <div className="text-[10px] text-blue-600 bg-blue-50 rounded px-2 py-1 mb-1">
