...
import { ListFieldEditor } from "../pickers/list-field-editor"
import { RichTextField } from "../pickers/rich-text-field"
import { ProductPicker } from "../pickers/product-picker"
...
function FieldRenderer({ field, value, onChange }: { field: FieldDef; value: unknown; onChange: (v: unknown) => void }) {
  ...
  switch (field.type) {
    ...
    case "textarea": {
      const isJsonField = field.label.includes("JSON") || field.name === "items" || field.name === "tiers" || field.name === "columns" || field.name === "rows"
      const jsonError = isJsonField && v ? (() => { try { JSON.parse(v); return null } catch (e) { return (e as Error).message } })() : null
      return (
        <div className="flex flex-col gap-1">
          <Textarea value={v} onChange={(e) => onChange(e.target.value)} rows={3} className={cn("text-xs min-h-[64px]", jsonError && "border-red-500/50 focus-visible:ring-red-500/30")} />
          {jsonError && <span className="text-[10px] text-red-400">Invalid JSON: {jsonError}</span>}
        </div>
      )
    }
              ...
              return (
                <div key={field.name} className="flex flex-col gap-1.5 group/field">
                  <div className="flex items-center gap-1.5">
                    ...
                    {override && <span className="text-[9px] bg-blue-500/10 text-blue-400 rounded px-1 py-px font-semibold">{override}</span>}
                    {isModified && (
