    ...
    <div className="absolute inset-0 pointer-events-none z-40">
      ...
      {isSelected && (
        <>
          ...
          {cols.length > 1 && <button className="absolute pointer-events-auto size-4 rounded-full bg-red-500 text-white text-[10px] flex items-center justify-center hover:bg-primary shadow-sm" style={{ left: ox + w + 4, top: oy + h / 2 + 8 }} onClick={() => removeTrack('col', cols.length - 1)}>−</button>}
          {rows.length > 1 && <button className="absolute pointer-events-auto size-4 rounded-full bg-red-500 text-white text-[10px] flex items-center justify-center hover:bg-primary shadow-sm" style={{ left: ox + w / 2 + 8, top: oy + h + 4 }} onClick={() => removeTrack('row', rows.length - 1)}>−</button>}
        </>
      )}
    </div>
  );
