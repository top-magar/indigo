...
export function BoxHandle({ element, id, prop, val, dir, sign, color, style, cls, h }: {
  ...
  return (
    <div className={cn('absolute z-[14]', cls)} style={adjustedStyle} onPointerDown={(e) => h.drag(element, id, prop, dir, sign, 4, e)} onPointerEnter={() => h.hover(id)} onPointerLeave={() => h.hover(null)}>
      ...
      {h.active === id && <span className={cn('absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-md px-1 py-px text-[10px] font-mono text-white whitespace-nowrap pointer-events-none z-20 shadow', color === 'emerald' ? 'bg-emerald-600' : 'bg-orange-500')} style={{ transform: 'translate(-50%,-50%) scale(calc(1 / var(--zoom, 1)))' }}>{val}px</span>}
    </div>
  );
}
