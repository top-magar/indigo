...
export function ProductPickerField({ label, tenantId, value, onChange }: ProductPickerFieldProps) {
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      {value && selected ? (
        <div className="flex items-center gap-2 p-2 rounded border" style={{ borderColor: 'var(--editor-border)' }}>
          ...
          <span className="flex-1 text-xs truncate" style={{ color: 'var(--editor-text)' }}>{selected.name}</span>
          <Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => onChange(null)}>✕</Button>
        </div>
      ) : (
        <Popover open={open} onOpenChange={(v) => { setOpen(v); if (!v) setSearch("") }}>
          <PopoverTrigger asChild>
            <Button variant="outline" className="h-auto py-2 text-xs font-normal border-dashed justify-start" style={{ color: 'var(--editor-text-secondary)' }}>
              Choose product…
            </Button>
          </PopoverTrigger>
