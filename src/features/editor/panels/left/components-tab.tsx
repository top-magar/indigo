      ...
      <div className="flex-1 overflow-y-auto px-2 pb-3">
        ...
        {filteredSaved.length > 0 && (
          <div className="mt-1">
            <button onClick={() => setCollapsed(p => ({ ...p, Saved: !p.Saved }))}
              ...
              <MIcon name="bookmark" size={13} className="text-sidebar-foreground/60" />
              <span>Saved</span>
              ...
              <div className="grid grid-cols-3 gap-1 px-0.5 pb-1">
                {filteredSaved.map((item) => (
                  <div key={item.id} draggable onDragStart={(e) => onDragSaved(item, e)}
                    ...
                    <span className="text-[8px] font-medium text-sidebar-foreground/60 truncate w-full text-center leading-tight">{item.name}</span>
                    <button onClick={(e) => { e.stopPropagation(); handleDeleteSaved(item.id); }}
                      className="absolute -top-1 -right-1 size-4 rounded-full bg-destructive/80 text-white items-center justify-center text-[8px] hidden group-hover/saved:flex">
        ...
        {q && !filteredSaved.length && groups.every(g => !g.items.some(i => i.label.toLowerCase().includes(q) || i.type.toLowerCase().includes(q))) && (
          <div className="py-12 text-center">
            ...
            <p className="text-[10px] text-sidebar-foreground/50">No results for &ldquo;{query}&rdquo;</p>
          </div>
        )}
      </div>
    </div>
