...
import { editorEmit } from "../stores/editor-events"
import { saveThemeAction } from "../actions/actions"
        ...
        {colorFields.map(({ key, label }) => (
          <div key={key} className="flex items-center gap-2 h-7">
            <label className="relative shrink-0 cursor-pointer">
              ...
              <div className="w-[22px] h-[22px] rounded border border-border" style={{ backgroundColor: theme[key] }} />
            </label>
  ...
  return (
    <div className="p-2.5 space-y-3">
      ...
      <div className="p-2 rounded border border-border bg-muted">
        <p className="m-0 font-bold text-foreground" style={{ fontFamily: theme.headingFont, fontSize: 14 * (theme.headingScale / 100), lineHeight: 1.2, letterSpacing: `${theme.headingLetterSpacing}em` }}>Heading Preview</p>
        <p className="mt-1 mb-0 text-muted-foreground" style={{ fontFamily: theme.bodyFont, fontSize: 12 * (theme.bodyScale / 100), lineHeight: theme.bodyLineHeight }}>Body text in {theme.bodyFont} at {theme.bodyScale}%</p>
      </div>
    </div>
  )
}
...
function LayoutSection({ theme, set }: { theme: ThemeState; set: SetFn }) {
  return (
    <div className="p-2.5 space-y-3">
      ...
      <div className="space-y-1">
        {[{ value: 960, label: "960px", desc: "Narrow" }, { value: 1152, label: "1152px", desc: "Default" }, { value: 1280, label: "1280px", desc: "Wide" }, { value: 1440, label: "1440px", desc: "Extra wide" }, { value: 0, label: "Full", desc: "No limit" }].map((opt) => (
      ...
      <div className="space-y-1">
        {(["solid", "outline", "ghost"] as const).map((s) => (
          <OptionBtn key={s} selected={theme.buttonStyle === s} onClick={() => set("buttonStyle", s)}
            label={s.charAt(0).toUpperCase() + s.slice(1)} desc={s === "solid" ? "Filled background" : s === "outline" ? "Border only" : "Text only"} />
        ))}
      </div>
...
function AnimationsSection({ theme, set }: { theme: ThemeState; set: SetFn }) {
  return (
    <div className="p-2.5 space-y-3">
      ...
      <div className="space-y-1">
        {([{ value: "none", label: "None", desc: "No hover animation" }, { value: "lift", label: "Vertical Lift", desc: "Cards shift up" }, { value: "3d-lift", label: "3D Lift", desc: "Cards lift forward" }, { value: "scale", label: "Scale", desc: "Cards grow slightly" }] as const).map((opt) => (
      ...
      <div className="space-y-1">
        {([{ value: "none", label: "None" }, { value: "fade", label: "Fade" }, { value: "slide-left", label: "Slide Left" }, { value: "slide-up", label: "Slide Up" }] as const).map((opt) => (
...
function AdvancedSection({ theme, set }: { theme: ThemeState; set: SetFn }) {
  return (
    <div className="p-2.5 space-y-3">
      ...
      <Textarea value={theme.customCss} onChange={(e) => set("customCss", e.target.value)}
        placeholder={".my-class {\n  color: red;\n}"} spellCheck={false} className="font-mono text-xs h-[120px] resize-y" />
      <p className="text-[11px] text-muted-foreground/60">Applies to your entire store.</p>
    </div>
  )
}
