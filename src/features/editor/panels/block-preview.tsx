...
export function BlockPreview({ name }: { name: string }) {
  switch (name) {
    ...
    case "Header":
      return <div className="flex w-full items-center gap-2 px-2"><div className="size-2 rounded bg-blue-200" /><div className={`${bar} w-6`} /><div className="ml-auto flex gap-1"><div className={`${bar} w-3`} /><div className={`${bar} w-3`} /></div></div>
    case "Footer":
    ...
    case "Testimonials": case "Social Proof": case "Testimonials — Cards":
      return <div className="flex w-full gap-1 px-2">{[0,1,2].map(i => <div key={i} className="flex flex-1 flex-col items-center gap-0.5"><div className="size-2 rounded-full bg-muted" /><div className={`${bar} w-full`} /></div>)}</div>
    case "Trust Signals":
      return <div className="flex w-full justify-center gap-2 px-2">{[0,1,2,3].map(i => <div key={i} className="size-3.5 rounded bg-muted" />)}</div>
    ...
    case "Video": case "Video Feature":
      return <div className="flex h-8 w-full items-center justify-center rounded bg-muted mx-2"><Play className="size-3.5 text-muted-foreground" /></div>
    ...
    case "Image": case "Image with Text": case "Slideshow":
      return <div className="flex h-8 w-full items-center justify-center rounded bg-muted mx-2"><ImageIcon className="size-3.5 text-muted-foreground" /></div>
    case "Button":
      return <div className="flex w-full justify-center p-2"><div className="h-3 w-14 rounded bg-blue-200" /></div>
    case "Contact Info":
      return <div className="flex w-full flex-col gap-0.5 px-2"><div className="flex items-center gap-1"><MapPin className="size-2 text-muted-foreground" /><div className={`${bar} w-10`} /></div><div className="flex items-center gap-1"><Mail className="size-2 text-muted-foreground" /><div className={`${bar} w-12`} /></div></div>
    case "Divider":
