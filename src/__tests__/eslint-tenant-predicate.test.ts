 ...
 * not.
 *
 * RuleTester.run() must be called at the top level. It registers its own
 * describe/it blocks against the global test hooks, so wrapping it in an it()
 * fails with "Calling the suite function inside test function is not allowed".
 */
