 ...
 * @returns The Vercel domain response
 */
...
export async function removeDomainFromVercel(domain: string): Promise<void> {
  ...
  // 404 is acceptable - domain may not exist in Vercel
 ...
 * Checks both domain configuration and SSL status
 * 
...
export async function getDomainStatus(domain: string): Promise<DomainStatus> {
  ...
  // Get domain configuration to check SSL status
  ...
  if (configResponse.ok) {
    ...
    // SSL is ready when domain is configured and verified
 ...
 * 
 * @param domain - The domain to check
 * @returns True if the domain is fully ready
 */
